...
def main():
    ...
    pascal_classifier = tf.estimator.Estimator(
        ...
        model_dir="./01_pascal_model_modified_info")
    ...
    for _ in range(100):
        ...
        #tf.identity(np.mean(gt_AP), name='Evaluation_mAP')
        #tf.summary.scalar('Evaluation_mAP', np.mean(gt_AP))
   ...
   # plt.plot(xrange(10),mAP_values)
   # plt.xlabel('timesteps')
   # plt.ylabel('mAP')
   # plt.show()
   # plt.savefig('01_pascal_modified_info')
    filen = open('mAP_01_pascal_modified.txt','w')
    for i in mAP_values:
        filen.write(str(i)+'\n')
    filen.close()
